...
public class MeetingRoomDao {
	...
	public MeetingRoom getMroom(int mId){
		SqlSession sqlSession = getSqlSessionFactory().openSession();
		MeetingRoom mroom = null;
		
		try {
			mroom = sqlSession.getMapper(MeetingRoomMapper.class).getMroom(mId);
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
			sqlSession.close();
	...
	public int delMroom(int mId){
		...
		try {
			re = sqlSession.getMapper(MeetingRoomMapper.class).delMroom(mId);
			if(re > 0){
				sqlSession.commit();
			}else{
				sqlSession.rollback();
